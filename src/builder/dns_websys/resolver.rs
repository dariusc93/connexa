...
mod tests {
    ...
    fn unquote_txt_concatenates_multi_segment() {
        // A >255-byte TXT record arrives as multiple quoted character-strings, which must be
        // concatenated with no separator — both space-separated and adjacent forms.
        assert_eq!(unquote_txt("\"part1\" \"part2\""), "part1part2");
        assert_eq!(unquote_txt("\"part1\"\"part2\""), "part1part2");
        // A dnsaddr multiaddr split across two segments must reassemble exactly.
        assert_eq!(
    ...
    fn unquote_txt_handles_escapes() {
        ...
        assert_eq!(unquote_txt("\"a\\\\b\""), "a\\b"); // \\ -> \
        assert_eq!(unquote_txt("\"\\097\""), "a"); // \DDD decimal -> byte
    }
